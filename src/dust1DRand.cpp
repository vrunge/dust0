...
// [[Rcpp::export]]
List dust1DRand(NumericVector data, double penalty = 0, double alpha = 1e-9) {
  ...
  for (int t = 2; t <= n; t++)
  {
    ...
    do
    {
      testValue = test1DRand(t, validIndices.read(), *validIndices.draw(), valuesCumsum, costRecord, optimalCost); // compute test value
